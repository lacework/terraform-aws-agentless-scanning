...
| <a name="input_management_account"></a> [management\_account](#input\_management\_account) | The AWS Account ID for the AWS Organizations management account. | `string` | `""` | no |
| <a name="input_monitored_accounts"></a> [monitored\_accounts](#input\_monitored\_accounts) | List of AWS Account IDs and Organizational Units for the AWS Organization integration type. | `list(string)` | `[]` | no |
| <a name="input_org"></a> [org](#input\_org) | Whether or not to create an AWS Organization scanning integration. Defaults to `false`. | `bool` | `false` | no |
